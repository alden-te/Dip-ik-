...
st.set_page_config(page_title="Ultra Pro Dip & Tepe DNA Analiz", page_icon="🧬", layout="wide")

st.title("🧬 Ultra Pro Dip & Tepe DNA Analiz Sistemi")
st.markdown("80-80 Pivot + 40+ MA + 100 Bar Öncesi Yol Analizi + İstatistiksel Sentez (Dip & Tepe)")
...
# ============================================================================
# BÖLÜM 3: 80-80 PIVOT VE 100 BAR ÖNCESİ YOL ANALİZİ (Dip & Tepe)
# ============================================================================
def find_80_80_pivots(df, is_peak=False):
    values = df['High'].values if is_peak else df['Low'].values
    n = len(values)
    pivots = []
    for i in range(80, n - 80):
        if is_peak:
            if np.all(values[i] > values[i-80:i]) and np.all(values[i] > values[i+1:i+81]):
                pivots.append(i)
        else:
            if np.all(values[i] < values[i-80:i]) and np.all(values[i] < values[i+1:i+81]):
                pivots.append(i)
...
def analyze_100bar_path(df, pivot_idx, is_peak=False):
    start_idx = max(0, pivot_idx - 100)
    path_window = df.iloc[start_idx : pivot_idx]
    pivot_bar = df.iloc[pivot_idx]
    pivot_price = float(pivot_bar['Close'])

    ema_vals = [float(pivot_bar.get(f'EMA_{p}', np.nan)) for p in [8, 13, 21, 50, 200, 600] if pd.notna(pivot_bar.get(f'EMA_{p}', np.nan))]
    sma_vals = [float(pivot_bar.get(f'SMA_{p}', np.nan)) for p in [20, 50, 100, 200, 800] if pd.notna(pivot_bar.get(f'SMA_{p}', np.nan))]
    ...
    is_bullish_align = 1 if (len(ema_vals) >= 6 and all(ema_vals[i] > ema_vals[i+1] for i in range(len(ema_vals)-1))) else 0

    low_20 = df['Low'].iloc[max(0, pivot_idx-19):pivot_idx+1].min()
    high_20 = df['High'].iloc[max(0, pivot_idx-19):pivot_idx+1].max()
    range_20 = high_20 - low_20
    price_pos = ((pivot_price - low_20) / range_20) * 100 if range_20 > 0 else 50

    path_stats = {
        'path_rsi_mean': float(path_window['RSI_14'].mean()),
        'path_rsi_min': float(path_window['RSI_14'].min()),
        'path_rsi_max': float(path_window['RSI_14'].max()),
        'path_rsi_below_30_days': int((path_window['RSI_14'] < 30).sum()),
        'path_rsi_above_70_days': int((path_window['RSI_14'] > 70).sum()),
    ...
    for i in range(len(path_window)):
        ...
        if len(e_vals) >= 6:
            if all(e_vals[j] < e_vals[j+1] for j in range(len(e_vals)-1)):
                bearish_days += 1
            if all(e_vals[j] > e_vals[j+1] for j in range(len(e_vals)-1)):
                bullish_days += 1
                
    path_stats['path_bearish_align_days'] = bearish_days
    path_stats['path_bullish_align_days'] = bullish_days

    return {
        'date': df.index[pivot_idx].strftime('%Y-%m-%d'),
        'price': pivot_price,
        'rsi': float(pivot_bar.get('RSI_14', 50)),
        'mfi': float(pivot_bar.get('MFI_14', 50)),
        'stoch': float(pivot_bar.get('STOCHRSIk_14', 50)),
        'willr': float(pivot_bar.get('WILLR_14', -50)),
        'ema_tangle': ema_tangle,
        'sma_tangle': sma_tangle,
        'is_bearish_align': is_bearish_align,
        'is_bullish_align': is_bullish_align,
        'price_pos': price_pos,
        'vol_ratio': float(pivot_bar.get('Vol_Ratio', 1.0)),
        'bbp': float(pivot_bar.get('BBP_20', 0.5)),
        **path_stats
    }

def process_stock(ticker, period="5y", analyze_peaks=False):
    try:
        ...
        # Dip Analizi
        pivots_dip = find_80_80_pivots(df, is_peak=False)
        if pivots_dip:
            dips = []
            for p_idx in pivots_dip:
                if pd.to_datetime(df.index[p_idx]).weekday() >= 5:
                    continue
                analysis = analyze_100bar_path(df, p_idx, is_peak=False)
                if analysis:
        ...
        # Tepe Analizi
        if analyze_peaks:
            pivots_peak = find_80_80_pivots(df, is_peak=True)
            if pivots_peak:
                peaks = []
                for p_idx in pivots_peak:
                    if pd.to_datetime(df.index[p_idx]).weekday() >= 5:
                        continue
                    analysis = analyze_100bar_path(df, p_idx, is_peak=True)
                    if analysis:
                        analysis['ticker'] = ticker
                        peaks.append(analysis)
                if peaks:
                    peaks_df = pd.DataFrame(peaks)
                    
        return df, dips_df, peaks_df
    except Exception:
        return None, None, None

# ============================================================================
# BÖLÜM 4: DİP & TEPE DNA SENTEZİ
# ============================================================================
...
def synthesize_peak_dna(peaks_df):
    if peaks_df is None or peaks_df.empty:
        return None
    dna = {
        'total_peaks': len(peaks_df),
        'peak_rsi_med': float(peaks_df['rsi'].median()),
        'peak_rsi_25': float(peaks_df['rsi'].quantile(0.25)),
        'peak_rsi_75': float(peaks_df['rsi'].quantile(0.75)),
        'peak_mfi_med': float(peaks_df['mfi'].median()),
        'peak_stoch_med': float(peaks_df['stoch'].median()),
        'peak_ema_tangle_med': float(peaks_df['ema_tangle'].median()),
        'peak_vol_ratio_med': float(peaks_df['vol_ratio'].median()),
        'peak_price_pos_med': float(peaks_df['price_pos'].median()),
        'peak_bullish_align_pct': float(peaks_df['is_bullish_align'].mean()) * 100,
        'path_rsi_mean': float(peaks_df['path_rsi_mean'].median()),
        'path_rsi_max': float(peaks_df['path_rsi_max'].median()),
        'path_rsi_above_70_avg': float(peaks_df['path_rsi_above_70_days'].mean()),
        'path_vol_ratio_med': float(peaks_df['vol_ratio'].median()),
        'path_vol_spike_avg': float(peaks_df['path_vol_spike_days'].mean()),
        'path_bullish_days_avg': float(peaks_df['path_bullish_align_days'].mean())
    }
    return dna
...
# ============================================================================
# BÖLÜM 5: CANLI TARAMA - SON 10 BAR (Dip & Tepe)
# ============================================================================
def scan_last_10_bars_for_signals(ticker, dna_dip, dna_peak=None, analyze_peaks=False):
    try:
        ...
        for i in range(max(0, len(df)-10), len(df)):
            ...
            price_decrease = ((close / current_price) - 1) * 100
            
            if price_increase >= 10.0 and price_decrease >= 10.0:
                continue
            ...
            if dna_dip:
                score_dip = 0
                if dna_dip['dip_rsi_25'] <= rsi <= dna_dip['dip_rsi_75']: score_dip += 25
                elif abs(rsi - dna_dip['dip_rsi_med']) < 10: score_dip += 15
                if abs(ema_tangle - dna_dip['dip_ema_tangle_med']) < 5: score_dip += 20
                elif ema_tangle < 10: score_dip += 10
                if abs(vol_ratio - dna_dip['dip_vol_ratio_med']) < 1.0: score_dip += 20
                if price_pos < 30: score_dip += 20
                if bbp < 0.2: score_dip += 15
                
                if score_dip >= 60:
                    dip_signals.append({
                        'date': date, 'price': round(close, 2), 'current_price': round(current_price, 2),
                        'change_pct': round(price_increase, 2), 'match_score': score_dip,
                        'rsi': round(rsi, 1), 'mfi': round(mfi, 1), 'stoch': round(stoch, 1),
                        'ema_tangle': round(ema_tangle, 2), 'vol_ratio': round(vol_ratio, 2), 'price_pos': round(price_pos, 1)
                    })
            
            # Tepe Skorlaması
            if analyze_peaks and dna_peak:
                score_peak = 0
                if dna_peak['peak_rsi_25'] <= rsi <= dna_peak['peak_rsi_75']: score_peak += 25
                elif abs(rsi - dna_peak['peak_rsi_med']) < 10: score_peak += 15
                if abs(ema_tangle - dna_peak['peak_ema_tangle_med']) < 5: score_peak += 20
                elif ema_tangle < 10: score_peak += 10
                if abs(vol_ratio - dna_peak['peak_vol_ratio_med']) < 1.0: score_peak += 20
                if price_pos > 70: score_peak += 20
                if bbp > 0.8: score_peak += 15
                
                if score_peak >= 60:
                    peak_signals.append({
                        'date': date, 'price': round(close, 2), 'current_price': round(current_price, 2),
                        'change_pct': round(price_decrease, 2), 'match_score': score_peak,
                        'rsi': round(rsi, 1), 'mfi': round(mfi, 1), 'stoch': round(stoch, 1),
                        'ema_tangle': round(ema_tangle, 2), 'vol_ratio': round(vol_ratio, 2), 'price_pos': round(price_pos, 1)
                    })
                    
        return (dip_signals if dip_signals else None), (peak_signals if peak_signals else None)
    except Exception:
        return None, None
...
with st.sidebar:
    ...
    if mode == "Tek Hisse Derin Analiz":
        ...
        if not ticker_input.endswith('.IS'):
            ticker_input += '.IS'
        analyze_peaks = st.checkbox("Tepe (Peak) Analizi de Yap", value=False)
        ...
        analyze_peaks = st.checkbox("Tepe (Peak) Sinyallerini de Tara", value=False)

    st.markdown("---")
    st.info("🧬 **Sistem Özellikleri:**\n- 20 SMA + 20 EMA (SMA 800, EMA 600 dahil)\n- 80-80 Pivot Tespiti (Dip & Tepe)\n- 100 Bar Öncesi Yol Analizi\n- Python 3.14 Uyumlu")

# MOD 1: TEK HİSSE DERİN ANALİZ
if mode == "Tek Hisse Derin Analiz":
    st.header(f"🔬 {ticker_input} Derin Dip & Tepe Analizi")
    if st.button("Analizi Başlat", type="primary"):
        with st.spinner(f"{ticker_input} verisi çekiliyor ve 40+ indikatör hesaplanıyor..."):
            df, dips_df, peaks_df = process_stock(ticker_input, period="10y", analyze_peaks=analyze_peaks)
            
            if dips_df is not None and not dips_df.empty:
                st.success(f"✅ {len(dips_df)} adet 80-80 pivot dibi bulundu!")
                dna_dip = synthesize_dip_dna(dips_df)
                col1, col2, col3, col4 = st.columns(4)
                col1.metric("Toplam Dip", dna_dip['total_dips'])
                col2.metric("Tipik Dip RSI", f"{dna_dip['dip_rsi_med']:.1f}")
                col3.metric("Ort. EMA Tangle", f"%{dna_dip['dip_ema_tangle_med']:.2f}")
                col4.metric("Bearish Hizalanma", f"%{dna_dip['dip_bearish_align_pct']:.1f}")
                
                st.markdown("---")
                tab1, tab2, tab3 = st.tabs([" Dip DNA Sentezi", "🛣️ 100 Barlık Yol Analizi", "📈 Grafik"])
                with tab1:
                    st.subheader("Bu Hisse İçin Çıkarılan Dip DNA'sı")
                    insights = generate_dna_insights(dna_dip, is_peak=False)
                ...
                with tab2:
                    ...
                    path_df = pd.DataFrame({
                        ...
                        'Değer': [
                            f"{dna_dip['path_rsi_mean']:.1f}",
                            f"{dna_dip['path_rsi_min']:.1f}",
                            f"{dna_dip['path_rsi_below_30_avg']:.1f} gün",
                            f"{dna_dip['path_vol_ratio_med']:.2f}x",
                            f"{dna_dip['path_vol_spike_avg']:.1f} gün",
            ...
            # Tepe Analizi Sonuçları
            if analyze_peaks and peaks_df is not None and not peaks_df.empty:
                st.success(f"✅ {len(peaks_df)} adet 80-80 pivot tepesi bulundu!")
                dna_peak = synthesize_peak_dna(peaks_df)
                col1, col2, col3, col4 = st.columns(4)
                col1.metric("Toplam Tepe", dna_peak['total_peaks'])
                col2.metric("Tipik Tepe RSI", f"{dna_peak['peak_rsi_med']:.1f}")
                col3.metric("Ort. EMA Tangle", f"%{dna_peak['peak_ema_tangle_med']:.2f}")
                col4.metric("Bullish Hizalanma", f"%{dna_peak['peak_bullish_align_pct']:.1f}")
                
                st.markdown("---")
                tab1, tab2 = st.tabs([" Tepe DNA Sentezi", "🛣️ 100 Barlık Yol Analizi (Tepe)"])
                with tab1:
                    st.subheader("Bu Hisse İçin Çıkarılan Tepe DNA'sı")
                    insights = generate_dna_insights(dna_peak, is_peak=True)
                    for insight in insights:
                        st.markdown(f"- {insight}")
                with tab2:
                    st.subheader("Tepeye Giden 100 Barlık Yolun İstatistikleri")
                    path_df = pd.DataFrame({
                        'Metrik': ['Ortalama RSI', 'En Yüksek RSI', 'RSI>70 Gün', 'Ort. Hacim', 'Hacim Patlaması', 'Bullish Gün'],
                        'Değer': [
                            f"{dna_peak['path_rsi_mean']:.1f}",
                            f"{dna_peak['path_rsi_max']:.1f}",
                            f"{dna_peak['path_rsi_above_70_avg']:.1f} gün",
                            f"{dna_peak['path_vol_ratio_med']:.2f}x",
                            f"{dna_peak['path_vol_spike_avg']:.1f} gün",
...
# MOD 2: ÇOKLU HİSSE TARAMA
    ...
    if st.button("Taramayı Başlat", type="primary"):
        ...
        for i, t in enumerate(tickers_list):
            try:
                df, dips_df, peaks_df = process_stock(t, period="5y", analyze_peaks=analyze_peaks)
                if dips_df is not None and not dips_df.empty:
        ...
        if all_dips:
            master_df_dip = pd.concat(all_dips, ignore_index=True)
            st.success(f"✅ Dip tarama tamamlandı! Toplam {len(master_df_dip)} dip analiz edildi.")
            master_dna_dip = synthesize_dip_dna(master_df_dip)
            st.subheader("🧬 Seçili Hisseler İçin Ortak Dip DNA Sentezi")
            insights = generate_dna_insights(master_dna_dip, is_peak=False)
            for insight in insights:
                st.markdown(f"- {insight}")
                
            st.subheader("🏆 En Çok Dip Oluşturan Hisseler")
            summary = master_df_dip.groupby('ticker').agg(
...
# MOD 3: TÜM BIST DNA SENTEZİ
    ...
    if st.button("Tüm BIST'i Tara", type="primary"):
        ...
        st.success(f"✅ Tarama tamamlandı! Süre: {elapsed:.0f} saniye.")
        ...
        if analyze_peaks and all_peaks:
            master_df_peak = pd.concat(all_peaks, ignore_index=True)
            st.subheader("🧬 Tüm BIST İçin Ortak Tepe DNA Sentezi")
            master_dna_peak = synthesize_peak_dna(master_df_peak)
            insights = generate_dna_insights(master_dna_peak, is_peak=True)
            for insight in insights:
                st.markdown(f"- {insight}")
...
# MOD 4: CANLI SİNYALLER (SON 10 BAR)
    ...
    if st.button("🔍 Canlı Sinyalleri Tara", type="primary", use_container_width=True):
        ...
        for i, t in enumerate(all_tickers[:100]):
            try:
                df, dips_df, peaks_df = process_stock(t, period="5y", analyze_peaks=analyze_peaks)
                if dips_df is not None and not dips_df.empty:
                    all_dips_for_dna.append(dips_df)
                if analyze_peaks and peaks_df is not None and not peaks_df.empty:
                    all_peaks_for_dna.append(peaks_df)
            except Exception:
                pass
                
        master_dna_dip = None
        master_dna_peak = None
        
        if all_dips_for_dna:
            master_df_dna = pd.concat(all_dips_for_dna, ignore_index=True)
            master_dna_dip = synthesize_dip_dna(master_df_dna)
            st.success(f"✅ Dip Master DNA oluşturuldu! {len(master_df_dna)} dip analiz edildi.")
            
        if analyze_peaks and all_peaks_for_dna:
            master_df_dna_peak = pd.concat(all_peaks_for_dna, ignore_index=True)
            master_dna_peak = synthesize_peak_dna(master_df_dna_peak)
            st.success(f"✅ Tepe Master DNA oluşturuldu! {len(master_df_dna_peak)} tepe analiz edildi.")
            
        if master_dna_dip or master_dna_peak:
            st.info("🔍 Şimdi son 10 barda sinyal veren hisseler aranıyor...")
            for i, t in enumerate(all_tickers):
                try:
                    dip_sig, peak_sig = scan_last_10_bars_for_signals(t, master_dna_dip, master_dna_peak, analyze_peaks)
                    if dip_sig:
                        live_dip_signals.append({'ticker': t, 'signals': dip_sig, 'latest_signal': dip_sig[-1]})
                    if peak_sig:
                        live_peak_signals.append({'ticker': t, 'signals': peak_sig, 'latest_signal': peak_sig[-1]})
                except Exception:
                    pass
                if i % 50 == 0:
                    progress_bar.progress((i + 1) / len(all_tickers))
            ...
            if live_dip_signals:
                st.success(f"✅ Dip tarama tamamlandı! Süre: {elapsed:.0f} saniye. **{len(live_dip_signals)} hisse** son 10 barda dip sinyali verdi!")
                st.subheader(f"🎯 Son 10 Barda Dip Sinyali Veren {len(live_dip_signals)} Hisse")
                for signal_data in live_dip_signals:
                    with st.expander(f"📈 {signal_data['ticker']} - {signal_data['latest_signal']['date']} - Skor: {signal_data['latest_signal']['match_score']}"):
                        ...
                        col3.metric("Değişim", f"%{latest['change_pct']}")
                        col4.metric("Eşleşme Skoru", f"%{latest['match_score']}")
                        
            if analyze_peaks and live_peak_signals:
                st.success(f"✅ Tepe tarama tamamlandı! **{len(live_peak_signals)} hisse** son 10 barda tepe sinyali verdi!")
                st.subheader(f"🎯 Son 10 Barda Tepe Sinyali Veren {len(live_peak_signals)} Hisse")
                for signal_data in live_peak_signals:
                    with st.expander(f"📉 {signal_data['ticker']} - {signal_data['latest_signal']['date']} - Skor: {signal_data['latest_signal']['match_score']}"):
                        latest = signal_data['latest_signal']
                        col1, col2, col3, col4 = st.columns(4)
                        col1.metric("Fiyat", f"{latest['price']} TL")
            ...
            if not live_dip_signals and (not analyze_peaks or not live_peak_signals):
                st.warning("️ Son 10 barda DNA'ya uyan sinyal bulunamadı.")
        else:
            st.error(" Master DNA oluşturulamadı. Lütfen tekrar deneyin.")

st.markdown("---")
st.markdown("<div style='text-align: center; color: gray;'><p>Ultra Pro Dip & Tepe DNA Analiz Sistemi v6.0 | Tüm BIST Hisseleri + Canlı Sinyal Tarama</p></div>", unsafe_allow_html=True)
